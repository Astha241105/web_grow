  ...
  async ({ quizId }, { rejectWithValue }) => {
    try {
      ...
      const response = await fetch(
        `http://webgrowproject-1.onrender.com/api/participant/quiz/${quizId}/results`,
        {
          method: 'GET',
          headers: {
...
export const fetchLeaderboard = createAsyncThunk(
  ...
  async ({ quizId }, { rejectWithValue }) => {
    try {
        const authToken = localStorage.getItem("authToken");
      const response = await fetch(
        `http://webgrowproject-1.onrender.com/api/participant/quiz/${quizId}/leaderboard-and-scores`,
        {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${authToken}`,
